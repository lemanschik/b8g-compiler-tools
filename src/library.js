...
`,
  ...
  emscripten_asm_const_int_sync_on_main_thread: (code, sigPtr, argbuf) => runMainThreadEmAsm(code, sigPtr, argbuf, 1),
...
function wrapSyscallFunction(x, library, isWasi) {
  ...
  if (!SYSCALLS_REQUIRE_FILESYSTEM && t.includes('FS.')) {
    t = modifyFunction(t, (name, args, body) => {
      return `(${args}) => {\n` +
  ...
  if (pre || post) {
    t = modifyFunction(t, (name, args, body) => `function (${args}) {\n${pre}${body}${post}}\n`);
  }
